graphics.off()
rm(list=ls(all=TRUE))
fileNameRoot="Figures/BernMetrop" # for output filenames
...
trajectory[1] = 0.99 #0.01 # arbitrary value
...
saveGraph( file=paste0( fileNameRoot , 
                        "SD" , proposalSD ,
                        "Init" , trajectory[1], "Posterior" ) , type="eps" )

# Open a plot, specifying height and width
openGraph(height=7,width=3.5)
# Create 2-rows layout for two plots
layout(matrix(1:2,nrow=2))
# Plot autocorrelation function of the accepted traj
acf( acceptedTraj , lag.max=30 , col="skyblue" , lwd=3 )
# Get accepted traj length
Len = length( acceptedTraj )
# Define lag
Lag = 10
# Define head of the traj, from 1 to Len-Lag
trajHead = acceptedTraj[ 1
                         : (Len-Lag) ]
# Define tail of the traj, from 1+Lag to Len
trajTail = acceptedTraj[ (1+Lag) : Len
]
# Scatterplot of the lagged trajectories
...
saveGraph( file=paste0( fileNameRoot , 
                        "SD" , proposalSD ,
                        "Init" , trajectory[1], "ACF", "Posterior" ) , type="eps" )
